...
int getNumSongs(const char* playlistName) {
    Playlist* p = findPlaylist(playlistName);
    return p->numSongs;
}
...
const char* getSongName(const char* playlistName, int index) {
    Playlist* p = findPlaylist(playlistName);
    return p->songs[index].name;
}
